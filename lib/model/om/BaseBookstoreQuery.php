...
abstract class BaseBookstoreQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = BookstorePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    Bookstore A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `NAME`, `LOCATION`, `WEBSITE` FROM `bookstore` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      $obj = new Bookstore();
      $obj->hydrate($row);
      BookstorePeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
